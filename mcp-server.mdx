...
| `probe_media` | Inspect a video/audio file — duration, dimensions, codec, fps, bitrate |
| `trim_video` | Cut a segment from a video by start/end time or start/duration |
| `resize_video` | Scale a video to new dimensions (contain, cover, or stretch) |
| `slice_video` | Split a video into segments with first-frame thumbnails |
| `run_ffmpeg` | Run any FFmpeg command in the cloud (arbitrary filters, concat, extract audio, etc.) |
