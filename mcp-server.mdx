...
On the consent screen you also pick **which account this connection works in** — your personal one or a team. Each client you connect keeps its own. See [Which account it works in](#which-account-it-works-in).
...
See [Which account it works in](#which-account-it-works-in).
...
`call_tool`, `render_video`, and `use_template` return an inline preview in your chat. It shows progress while the job runs, then picks a player from the result:
