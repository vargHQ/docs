...
<Step title="Create a .cursorrules file">
  ...
  - Images: `varg.imageModel("nano-banana-pro")` (versatile), `varg.imageModel("flux-pro")` (quality)
  - Video: `varg.videoModel("kling-v3")` (best quality), `varg.videoModel("wan-2.5")` (characters)
  ...
  3. Never write `<Image prompt="..." />` — it's `Image({ prompt: "..." })`
  4. One image per Video: `Video({ prompt: { images: [img] } })`
