...
| **varg** | FFmpeg locally, Rendi (cloud FFmpeg) in production, or cloud render (`POST /v2/render`). No headless Chrome. | Render service auto-scales ffmpeg workers; gateway caches AI generations to R2. | AI model calls happen during composition. Cache is sha256-keyed and free on hit. |
