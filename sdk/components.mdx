...
| `<Video>` | AI or source video | `prompt`, `src`, `duration`, `keepAudio` |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `duration` | `number` or `"auto"` | Auto | Clip timeline duration in seconds. When omitted or set to `"auto"`, the renderer derives it from the base video when possible |
...
| `duration` | `number` | `5` | Requested duration for generated video. Use `Clip.duration` to control composition timing |
| `volume` | `number` | `1` | Audio volume (0-1) |
| `keepAudio` | `boolean` | `true` | Preserve the video's source audio. Set to `false` to mute it |
| `cutFrom` | `number` | `0` | Start trim |
| `cutTo` | `number` | - | End trim |

`Video.duration` controls AI generation length. `Clip.duration` controls how long the clip appears in the final composition. For source videos, use `Clip.duration`, `cutFrom`, and `cutTo` for timeline and trimming behavior.
...
| `keepAudio` | `boolean` | `true` | Preserve audio from a video child. Set to `false` to mute it |
